 ...
 * @param {boolean} end if true the function will search at the end of the word
 * @returns {boolean}
 */
 ...
 */
 function mergeGreekReportAndPTX(greekReport, ptx, relaxed=true) {
    let merged = [];
    let ptxWords = null;
    ...
    greekReport.forEach((chapt, ic) => {
        ...
        chapt.forEach((verse, iv) => {
            ...
            verse.forEach((w, iw) => {
                ...
                if(relaxed || (ptxWordStrongs && ptxWordStrongs.filter(x => x===w["strong"]).length === w["occurencesLemma"])) {
                    ptxWords.forEach((pw, ipw) => {
                        if(!pw || ipw == 0) return;
                        if((relaxed 
                            && pw["pos"] == w["wordPos"]
                            && pw["occurence"] === w["occurence"]
                            && pw["occurences"] === w["occurences"])
                            || (pw["strong"] === w["strong"]
...
const generateReference = function(chapterInt, verseInt) {
    let cc = chapterInt.toString();
    let cv = verseInt.toString();
    if(chapterInt > 0 && chapterInt < 10) {
        cc = "00"+cc;
    } else if (chapterInt >= 10 && chapterInt < 100) {
        cc = "0"+cc;
    }

    if(verseInt > 0 && verseInt < 10) {
        cv = "00"+cv;
    } else if (verseInt >= 10 && verseInt < 100) {
        cv = "0"+cv;
    }
...
const mergeAlignReportActions = {
    startDocument: [
        {
            description: "setup",
            test: () => true,
            action: ({ workspace, config, output }) => {
                const { report, strippedSource } = config;
                // workspace.handler = new utils.PtxHandler(strippedSource);
    ...
    text: [
        {
            ...
            test: ({ workspace, config }) => !workspace.doNotCareAboutText && workspace.isInVerse && workspace.arrayWords && workspace.arrayWords.length > 0 && config.report[workspace.ref] != null,
    ...
    mark: [
        ...
        },
        {
            ...
            action: ({ config, context, workspace, output }) => {
                ...
                workspace.ref = generateReference(workspace.chapter,workspace.verse);
                ...
                const verseRecords = reportRecordsForCV(workspace.merged, workspace.chapter, workspace.verse);
                ...
                workspace.arrayWords = workspace.merged[workspace.chapter][workspace.verse];
...
const mergeAlignReport = function ({ perf, report, strippedSource }) {
    const actions = mergeActions(
        [
            mergeAlignReportActions,
    ...
    const output = {};
    cl.renderDocument({ docId: "", config: { report, strippedSource }, output });
    return { perf: output.perf, reportgreekptx: output.reportgreekptx, issues: output.issues };
}

const mergeReportAndPerf = {
    name: "mergeReportAndPerf",
    type: "Transform",
    description: "PERF=>PERF: Merge the alignmentReport and the strippedAlignment into the targetPerf input",
    inputs: [
        {
            name: "perf",
            type: "json",
    ...
    code: mergeAlignReport
}
export default mergeReportAndPerf;
